import { pgTable, serial, varchar, integer, timestamp, boolean, text, primaryKey, uniqueIndex } from 'drizzle-orm/pg-core';
...
export const movies = pgTable('movies', {
  ...
  ratingValue: varchar('rating_value', { length: 10 }).default('4.5'), // e.g. "8.9" or "4.5"
...
export const screens = pgTable('screens', {
  ...
  theatreId: integer('theatre_id').references(() => theatres.id, { onDelete: 'cascade' }).notNull(),
});
...
export const shows = pgTable('shows', {
  ...
  screenId: integer('screen_id').references(() => screens.id, { onDelete: 'cascade' }).notNull(),
...
export const bookings = pgTable('bookings', {
  ...
  code: varchar('code', { length: 100 }).notNull().unique(), // Unique ticket code
  createdAt: timestamp('created_at').defaultNow().notNull(),
});

// BOOKING SEATS (Link table)
export const bookingSeats = pgTable('booking_seats', {
  ...
  return {
    pk: primaryKey({ columns: [table.bookingId, table.seatId] }),
  };
});
...
export const groupMembers = pgTable('group_members', {
  id: serial('id').primaryKey(),
  roomId: integer('room_id').references(() => groupRooms.id, { onDelete: 'cascade' }).notNull(),
  userId: integer('user_id').references(() => users.id, { onDelete: 'cascade' }).notNull(),
...
export const votes = pgTable('votes', {
  ...
  votedId: integer('voted_id').notNull(), // id of the movie, theatre, or showtime
});
...
// DRIZZLE RELATIONS (Suited for DBMS Viva explanation)
...
export const screensRelations = relations(screens, ({ one, many }) => ({
  theatre: one(theatres, { fields: [screens.theatreId], references: [theatres.id] }),
  shows: many(shows),
  seats: many(seats),
}));
...
export const groupRoomsRelations = relations(groupRooms, ({ one, many }) => ({
  creator: one(users, { fields: [groupRooms.creatorId], references: [users.id] }),
  members: many(groupMembers),
  votes: many(votes),
}));

export const groupMembersRelations = relations(groupMembers, ({ one }) => ({
  room: one(groupRooms, { fields: [groupMembers.roomId], references: [groupRooms.id] }),
